...
class Config:
    dropout: float = 0.5
    num_layers: int = 2
    model_name: str = "clip-Vit-B/32"  # smaller than ViT-L/14
    device: torch.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    batch_size: int = 8                # reduce for Colab memory
    encoder: str = 'text'              # keep "text" unless training both modalities
    laion_type: str = 'laion_combined'
    transform: str = 'targetpad'
    target_ratio: float = 1.25
    learning_rate: float = 1e-4
    weight_decay: float = 0.05
    adam_epsilon: float = 1e-8
    num_epochs: int = 5                # shorter runs
    save_best: bool = True
    use_amp: bool = True
    validation_frequency: int = 1
    comment: str = "colab_run_clipB32_combined"
    dataset: str = 'cirr'
    save_path_prefix = "/content/drive/MyDrive/Zero-shot/model/checkpoints"
    eval_load_path: str = "/content/drive/MyDrive/Zero-shot/model/checkpoints/best.pth"
    submission_name: str = 'colab_cirr_test_run'
    skip_eval = True               # new boolean flag
    dataset = "laion"              # or whichever indicates training‐only
    checkpoint_dir = "/content/drive/MyDrive/Zero-shot/model/"  # where to save
